...
public final class USBMonitor {
    ...
    private static final String ACTION_USB_PERMISSION_BASE = "ai.aloft.libuvcamera.USB_PERMISSION.";
    ...
    public synchronized void register() throws IllegalStateException {
        ...
        if (mPermissionIntent == null) {
            ...
            if (context != null) {
                // needs to be mutable in order for result to go to intent
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
    ...
    private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            Log.d(TAG, "onReceive: Action: " + action + "ACTION_USB: " + ACTION_USB_PERMISSION);
            if (ACTION_USB_PERMISSION.equals(action)) {
                // when received the result of requesting USB permission
                synchronized (USBMonitor.this) {
                    final UsbDevice device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                    Log.d(TAG, "onReceive: Checking extra");

                    if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                        if (device != null) {
                            Log.d(TAG, "onReceive:processConnect");
                        ...
                        Log.d(TAG, "onReceive:processCancel");
