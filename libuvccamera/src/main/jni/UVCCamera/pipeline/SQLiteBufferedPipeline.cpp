...
SQLiteBufferedPipeline::SQLiteBufferedPipeline(const char *database_name, const bool &clear_table)
    ...
    sql_query_oldest_10 = new sqlite3pp::query(*db,
                                               "SELECT " ALL_FIELDS " FROM " TABLE_NAME " ORDER BY dtime LIMIT 10;");
    ...
    if (LIKELY(frame)) {
        ...
        for (; LIKELY(isRunning());) {
            ...
            if (LIKELY(isRunning())) {
                ...
                if (next_pipeline) {
                    queued_ids.clear();
                    for (auto iter = sql_query_oldest_10->begin();
                         iter != sql_query_oldest_10->end(); ++iter) {
                        if (UNLIKELY(!isRunning())) {
                            break;
                        }
                        // 0:id, 1:dtime, 2:format, 3:width, 4:height, 5:sequence, 6:data_bytes, 7:data
                        const int64_t id = (*iter).get<int64_t>(
                                0);    // XXX int64_t(long long int) may be better
...
//								LOGI("chain_frame:%lld,%lld,(%d,%d),actual_bytes=%d", id, dtime, width, height, actual_bytes);
                            ...
                            LOGW("uvc_ensure_frame_size failed:%lld,%lld,(%d,%d),actual_bytes=%d",
...
static JNINativeMethod methods[] = {
        ...
        {"nativeSetPipeline", "(JLai/aloft/libuvcamera/core/usb/IPipeline;)I", (void *) nativeSetPipeline},
...
int register_sqlite_buffered_pipeline(JNIEnv *env) {
    ...
    if (registerNativeMethods(env,
                              "ai/aloft/libuvcamera/core/usb/SQLiteBufferedPipeline",
