...
/* public */
PublisherPipeline::PublisherPipeline(const size_t &_data_bytes, const char *addr,
                                     const char *_subscription_id)
...
//	header.presentation_time_us_le = htole64(systemTime() / 1000);
    header.presentation_time_us_le = htole64(
            nsecs_t(frame->capture_time.tv_sec) * 1000000LL + nsecs_t(frame->capture_time.tv_usec));
...
//	LOGD("sub_str=%s,sub_sz=%d", sub_str, sub_sz);
    ...
    memcpy(((uint8_t *) payload.data()) + sizeof(publish_header_t), frame->data,
           frame->actual_bytes);
/*
...
int register_publisher_pipeline(JNIEnv *env) {
    ...
    if (registerNativeMethods(env,
                              "ai/aloft/libuvcamera/core/usb/PublisherPipeline",
