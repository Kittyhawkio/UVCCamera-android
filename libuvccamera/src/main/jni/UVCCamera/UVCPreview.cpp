        ...
        if (!env->IsSameObject(mFrameCallbackObj, frame_callback_obj)) {
            ...
            if (frame_callback_obj) {
                ...
                if (!iframecallback_fields.onFrame) {
                    LOGE("Can't find IFrameCallback#onFrame preview");
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
    ...
    if (LIKELY(frame)) {
        if (mFrameCallbackObj) {
            // Allocate a direct ByteBuffer and copy frame data into it
            jobject byteBuffer = env->NewDirectByteBuffer(frame->data, frame->data_bytes);

            // Call the Java callback method with the ByteBuffer
            env->CallVoidMethod(mFrameCallbackObj, iframecallback_fields.onFrame, byteBuffer);

            // Clear any exceptions (just like in the original code)
            env->ExceptionClear();

            // Delete the local reference to the ByteBuffer
